...
class EnrollmentApplicationControllerWebMvcTest {
  ...
  void reject_returnsBadRequestForBlankReason() throws Exception {
    ...
    verify(rejectEnrollmentApplicationUseCase, never()).execute(any(), any(), any());
  }

  @Test
  @DisplayName("Should forbid rejecting without reject permission")
  void reject_returnsForbiddenWithoutPermission() throws Exception {
    final var authentication = authentication();
    stubPermission(PermissionCode.ENROLLMENT_APPLICATION_REJECT, false);
  ...
  private JwtAuthenticatedUser authentication() {
    return institutionalPrincipal(UUID.randomUUID(), INSTITUTION_ID);
  }

  private void stubPermission(PermissionCode permission, boolean allowed) {
    final var authentication = new TestingAuthenticationToken(authentication(), null);
